...
private ProblemService problemService;
	...
	public Result newlist(@PathVariable String label,@PathVariable int page,@PathVariable int size){
		Page<Problem> result = problemService.newlist(label,page,size);
			return new Result(true,StatusCode.OK,"查询成功",new PageResult<Problem>(result.getTotalElements(),result.getContent()));
	}
	...
	public Result hotlist(@PathVariable String label,@PathVariable int page,@PathVariable int size){
		Page<Problem> result = problemService.hotlist(label,page,size);
		return new Result(true,StatusCode.OK,"查询成功",new PageResult<Problem>(result.getTotalElements(),result.getContent()));
	}
	...
	public Result waitlist(@PathVariable String label,@PathVariable int page,@PathVariable int size){
		Page<Problem> result = problemService.waitlist(label,page,size);
		return new Result(true,StatusCode.OK,"查询成功",new PageResult<Problem>(result.getTotalElements(),result.getContent()));
	}
